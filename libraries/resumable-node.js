...
module.exports = resumable = function(temporaryFolder){
  ...
  var getChunkFilename = function(chunkNumber, identifier){
    ...
    let filename = path.join($.temporaryFolder, './resumable-'+identifier+'.'+chunkNumber);
...
//console.log(filename , identifier ,totalSize ,chunkSize ,chunkNumber )
//console.log(validateRequest(chunkNumber, chunkSize, totalSize, identifier, filename))
...
//console.log(exists)
  ...
  // 2 : 'partly_done', filename, original_filename, identifier
  // 1 : 'done', filename, original_filename, identifier
  // 3 : 'invalid_resumable_request', null, null, null
  // 4 ; 'non_resumable_request', null, null, null
  /*
  change: add doneCallback, custom callback to 
            -return list of files names incase of all chunks uploaded
            - ['chunk file name']
            - or -1 if error happened
  notice: this function has a part that checks for all chunks if exist then return state 'done' otherwise 'partly_done', or error happened then 'invalid'
   */
  $.post = function(req, callback, doneCallback) {
...
/*
              ...
              try {
                ...
                /*fs.access('uploads/fsfs.jfs', fs.constants.F_OK, (err) => {
                  //console.log (err);
              ...
              fs.exists(chunkfileName, function(exists) {
                ...
                if(exists){
                  ...
                  if(currentTestChunk>numberOfChunks) {
                    //console.log ('finished prev file name: ', getChunkFilename(currentTestChunk-1, identifier))
                    callback(1, numberOfChunks, filename, original_filename, identifier);
                    //doneCallback(numberOfChunks, original_filename, identifier, filename)
                  } else {
                    // Recursion
                    //console.log ('recursion', numberOfChunks, currentTestChunk)
                    testChunkExists();
                  }
                } else {
                  callback('partly_done', filename, original_filename, identifier, chunkFilename);
                }
              });
              */
            }
        testChunkExists();
          ...
          callback(validation, numberOfChunks, filename, original_filename, identifier);
          //doneCallback(-1);
    }
  }
